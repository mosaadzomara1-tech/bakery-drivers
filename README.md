...
## لوحة الإدارة (جوّه نفس التطبيق)

برنامج واحد للكل: تطبيق السواقين فيه **وضع الإدارة** — من القائمة ⋮ ← «🔐 لوحة الإدارة»
(أو «أنا المدير» في أول شاشة)، والدخول بإيميل وباسورد Traccar. فيه «جرّب ببيانات تجريبية» كمان.

- **مباشر:** في الدوام / الإرسال متأخر / خارج الدوام، وساعات اليوم لكل سوّاق وللكل. بيتحدّث كل ٣٠ ثانية.
- **تنبيهات:** مابعتش موقع من ٣ دقايق أو أكتر، دوام أطول من ١٤ ساعة (غالباً نسي يقفل)، بطارية ١٥٪ أو أقل.
- **الخريطة:** كل السواقين بألوان حالتهم.
- **تفاصيل السوّاق:** دوامات أي يوم، الساعات والمسافة، المسار، وآخر موقع على خرائط جوجل.
- **الساعات:** سوّاق × يوم لأي فترة، وتصدير CSV يفتح في Excel للرواتب.
- **ملخص النهارده:** جاهز للواتساب بزرار واحد.

### التوصيل بـ Traccar
وضع الإدارة بيكلّم **واجهة Traccar (API)**، ودي على منفذ لوحة Traccar (الافتراضي `8082`)، مش منفذ المواقع (`5055`).
- لو الـ tunnel بتاعك على `8082`: اكتب نفس العنوان وادخل.
- لو على `5055`: اعمل tunnel تاني لـ `8082` وحط عنوانه في `config.json` باسم `"admin"`، والتطبيق هيملاه لوحده.

**ملاحظات:** الساعات = من أول موقع لآخر موقع (سكوت أكتر من ١٠ دقايق = دوام جديد). اضبط المنطقة الزمنية في إعدادات
Traccar عشان تقرير الأيام يتقسم صح. «افتكرني» بيحفظ الباسورد على الموبايل ده بس.
